...
- `region` (also accepted as `region_name`): AWS region for S3 and Glue, aliased to iceberg-go's `s3.region`/`glue.region` properties. Taken from the URI first, then from the ambient environment (`AWS_REGION` or the selected profile). If it still cannot be resolved, the AWS SDK will error, since a region is required — there is no `us-east-1` fallback.
