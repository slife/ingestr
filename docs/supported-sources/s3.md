...
To integrate `ingestr` with Amazon S3, you can provide an `access_key_id`/`secret_access_key`, or rely on the AWS default credential chain (see [URI Format](#uri-format)).
...
### Example: Using the default credential chain / IRSA

If your environment already provides AWS credentials ambiently (e.g. an EKS pod using IRSA, an EC2 instance profile, or environment variables), you can omit `access_key_id`/`secret_access_key` entirely:

```sh
ingestr ingest \
    --source-uri 's3://my-bucket' \
    --source-table 'my_bucket/students/students_details.csv' \
    --dest-uri duckdb:///s3_data.duckdb \
    --dest-table 'processed_students.student_details'
```
...
> [!NOTE]
> With no keys in the URI, ingestr authenticates as whatever AWS identity the environment provides (IRSA/pod role, instance profile, `AWS_*` environment variables, or the selected `profile`). Make sure that ambient identity is the one you intend and is scoped to only the buckets this ingestion needs — the access it grants is the blast radius.
